...
def test_impading_L(model, L, refield=8, sf=1, refield_extension=0):
    '''
    model:
    L: input Low-quality image
    refield: effective receptive filed of the network, 32 is enough
    ...
    '''
    # padding
    h_ori, w_ori = L.size()[-2:]
    h    = h_ori - refield_extension
    w    = w_ori - refield_extension

    paddingBottom = int(ceil(h/refield)*refield - h)
    paddingRight  = int(ceil(w/refield)*refield - w)
    L = F.pad(L, [0, paddingRight, 0, paddingBottom], mode='circular')

    #L = torch.cat([L, torch.flip(L, [2])], 2)[:, :, :h_ori + paddingBottom, :]
    #L = torch.cat([L, torch.flip(L, [3])], 3)[:, :, :, :w_ori + paddingRight]

    E = model(L)
    if isinstance(E, tuple):
        E = E[0]

    E = E[..., :(h+refield_extension)*sf, :(w+refield_extension)*sf]

    return E

def test_impading_LS(model, L, S, refield=8, sf=1, refield_extension=0):
    '''
    model:
    L: input Low-quality image
    refield: effective receptive filed of the network, 32 is enough
...
def test_impading_LSG(model, L, S, G, refield=8, sf=1, refield_extension=0):
    '''
    model:
    L: input Low-quality image
    refield: effective receptive filed of the network, 32 is enough
    ...
    # padding
    h, w = L.size()[-2:]
    h    = h - refield_extension
    w    = w - refield_extension

    paddingBottom = int(ceil(h/refield)*refield - h)
    paddingRight  = int(ceil(w/refield)*refield - w)
    L = F.pad(L, [0, paddingRight, 0, paddingBottom], mode='circular')
    G = F.pad(G, [0, paddingRight, 0, paddingBottom], mode='circular')
